...
class MCT():
    ...
    def __init__(self,maxLayer=12,maxNum=10,ensemble=False,C=2,adaptC=False):
    ...
    def BPIter(self,myNode,C=500):
        if self.adaptC==False:
            ...
            while tmpNode.parent is not None:
                #-update the ucb while BP
                tmpNode.ucb=tmpNode.value+C*np.sqrt(np.log(tmpNode.parent.checked)/tmpNode.checked)

                #-update the usb of the cousins of the tmpNode
                for childItem in tmpNode.parent.childList:
                    if tmpNode is not self.nodeList[childItem]:
                        self.nodeList[childItem].ucb=self.nodeList[childItem].value+C*np.sqrt(np.log(self.nodeList[childItem].parent.checked)/self.nodeList[childItem].checked)
                        ...
                        if decisionNodeItem.ucb==maxUCB:
                            ...
                            while tmpTravelNode.parent is not None:
                                if type(tmpTravelNode)==StatusNode:
                                    #-----status node
                                    tmpTravelNode.value=(tmpTravelNode.value*tmpTravelNode.checked+tmpTravelNode.parent.decision*tmpTravelNode.status[0]+tmpTravelNode.parent.decision*tmpTravelNode.status[1]+tmpTravelNode.parent.decision*tmpTravelNode.status[2]+tmpAggValue)/(tmpTravelNode.checked+1)+np.sum([self.nodeList[childI].value*self.nodeList[childI].checked/tmpTravelNode.checked for childI in tmpTravelNode.childList])
                                else:
                                    #-----decision node
                                    tmpMean=np.sum([self.nodeList[childI].value*self.nodeList[childI].checked/tmpTravelNode.checked for childI in tmpTravelNode.childList])
                                    tmpStd=np.sqrt(np.sum([(self.nodeList[childI].value-tmpMean)**2*self.nodeList[childI].checked/tmpTravelNode.checked for childI in tmpTravelNode.childList]))
                                    tmpTravelNode.value=(tmpTravelNode.value*tmpTravelNode.checked+tmpMean-tmpStd)/(tmpTravelNode.checked+1)
                            ...
                            if tmpStatusNode not in newStatusList\
                                ...
                                self.BPIter(tmpStatusNode,C=self.C/(10**(layerI+1)))
    ...
    def predictItem(self,statusListStr,clayer,myNode=None,preIter=20):
        if len(statusListStr)==0:
            ...
            valueList=[self.nodeList[childI].value for childI in self.nodeList[0].childList]
            ...
            if statusNode is not None:
                ...
                decisionList=[self.nodeList[childI].value for childI in statusNode.childList]
                ...
                #--expand the tree
                self.training(X,y,startLayer=clayer+1,incrementalNode=statusNode,maxIter=preIter)
                ...
                for childI in statusNode.childList:
                    ...
                    if np.sum(statusNode.status[:3])>0:
                        ...
                        #----status==[0,0,0]
                        try:
                            maxValue=max([self.nodeList[childI].value for childI in statusNode.childList])
                            if tmpDecisionNode.value==maxValue:
                                decision=tmpDecisionNode.decision
                                print("卑微预测：",decision)
                                return tmpDecisionNode,decision
...
if __name__=="__main__":
    ...
    y=np.array(trainDf.loc[:,"price"])
    myMCT=MCT(C=C,adaptC=adaptC)
    myMCT.training(X,y,maxIter=500)
