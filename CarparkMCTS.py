...
def BuildTree(tree, node, k):
    ...
    for d in range(k):
        ...
        if nH>0:
            ...
            for i in range(nH):
                while tree['DayOfWeek'][hourly_data['Weekday'][day].values[search]]==0:
                    search=search+1
...
def integrateTree(myNode):
        oriDict=weekday_tree
        myDict={}
        if oriDict[myNode]["Type"]=="D":
            myDict["name"]="{}:{},{},{:2f}".format(myNode,oriDict[myNode]["Type"],oriDict[myNode]["State"],oriDict[myNode]["V"])
        else:
            myDict["name"]="{}:{},{:.2},{:2f}".format(myNode,oriDict[myNode]["Type"],oriDict[myNode]["UCB"],oriDict[myNode]["V"])
        myDict["children"]=[]
        if len(weekday_tree[myNode]["Child"])!=0:
            for child in weekday_tree[myNode]["Child"]:
                myDict["children"].append(integrateTree(child))
        return myDict
